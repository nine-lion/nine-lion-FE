...
function CalendarTab({ accountKey }: { accountKey: string }) {
  ...
  const addNaturalEntry = async (event: FormEvent) => {
    ...
    try {
      ...
      const conflicting = newBlocks.filter((block) => hasOverlap(blocks, block.date, block.start, block.end));
      if (conflicting.length > 0) {
        setMessage('겹치는 시간대가 있어 추가할 수 없어요.');
        setSubmitting(false);
        return;
      }
  ...
  const updateBlock = (id: number, patch: Omit<TimeBlock, 'id' | 'date'>) => {
    ...
    setBlocks((previous) => previous.map((block) => {
      ...
      const candidate = { ...block, ...patch };
      if (hasOverlap(previous, candidate.date, candidate.start, candidate.end, id)) {
        setMessage('겹치는 시간대가 있어 저장할 수 없어요.');
        return block;
      }
      return candidate;
    }));
  ...
  const createBlock = (block: Omit<TimeBlock, 'id'>) => {
    if (hasOverlap(blocks, block.date, block.start, block.end)) {
      setMessage('겹치는 시간대가 있어 추가할 수 없어요.');
      setCreating(false);
      return;
    }
      ...
      <Tabs defaultValue="planner" className="app-tabs">
        ...
        <div className="content-wrap">
          ...
          <TabsContent value="analysis">
            <AnalysisTab key={accountKey} accountKey={accountKey} />
          </TabsContent>
        </div>
      </Tabs>
    </main>
